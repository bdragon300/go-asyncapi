...
func (pc ProtoChannel) RenderDefinition(ctx *common.RenderContext) []*j.Statement {
	...
	res = append(res, pc.renderWSMethods(ctx)...)
	if pc.Publisher {
		res = append(res, pc.RenderCommonPublisherMethods(ctx, pc.Struct)...)
		res = append(res, pc.renderWSPublisherMethods(ctx)...)
	}
	if pc.Subscriber {
...
func (pc ProtoChannel) String() string {
	return "WS ProtoChannel " + pc.Name
}
...
func (pc ProtoChannel) renderWSMethods(ctx *common.RenderContext) []*j.Statement {
	ctx.Logger.Trace("renderWSMethods", "proto", pc.ProtoName)
	return nil
}

func (pc ProtoChannel) renderWSPublisherMethods(ctx *common.RenderContext) []*j.Statement {
	ctx.Logger.Trace("renderWSPublisherMethods", "proto", pc.ProtoName)
			...
			BlockFunc(func(bg *j.Group) {
				...
				if pc.PubMessagePromise != nil && pc.PubMessagePromise.Target().HasProtoBindings(pc.ProtoName) {
					bg.Op("envelope.SetBindings").Call(
						j.Add(utils.ToCode(pc.PubMessagePromise.Target().BindingsStruct.RenderUsage(ctx))...).Values().Dot("WS()"),
					)
				}
				bg.Return(j.Nil())
			}),
