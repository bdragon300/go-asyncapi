...
func (pb ProtoBuilder) BuildServerBindings(ctx *common.CompileContext, rawData types.Union2[json.RawMessage, yaml.Node]) (vals *lang.GoValue, jsonVals types.OrderedMap[string, string], err error) {
	...
	vals = lang.ConstructGoValue(bindings, []string{"LastWill"}, &lang.GoSimple{OriginalName: "ServerBindings", Import: ctx.RuntimeModule(pb.ProtoName)})
	if bindings.LastWill != nil {
		vals.StructValues.Set("LastWill", lang.ConstructGoValue(*bindings.LastWill, []string{}, &lang.GoSimple{OriginalName: "LastWill", Import: ctx.RuntimeModule(pb.ProtoName)}))
	}
