...
func cliMerge(cmd *MergeCmd, cmdConfig common2.ToolConfig) error {
	...
	var enc anyEncoder
	switch cmdConfig.Doc.Merge.Format {
	case "json":
	...
	case "yaml":
		...
		enc = e
	default:
		return fmt.Errorf("%w: unknown output format %q", common2.ErrWrongCliArgs, cmd.Format)
	}
	...
	if outputPath == "" {
		...
		outputPath = fmt.Sprintf("merged_%s_%x.%s", strings.TrimSuffix(cmd.Documents[0], path.Ext(cmd.Documents[0])), sum, cmdConfig.Doc.Merge.Format)
	}
