...
func cliGenExamples(cmd *GenExamplesCmd, cmdConfig common2.ToolConfig) error {
	...
	for _, t := range targets {
		...
		if examplesNode.Kind() != types.RawNodeKindArray {
			logger.Warn("Existing examples subnode is not an array, skipping", "path", t, "kind", examplesNode.Kind())
			break
		}
		if (hasExample && examplesNode.Len() > 0) && !cmdConfig.Doc.GenExamples.Append {
			logger.Debug("Skipping a node because it already contains the examples", "path", t)
			continue
		}

		entity := asyncapiEntityByPath(t.Path())

		logger.Debug("Generating examples", "path", t, "entity", entity, "count", cmdConfig.Doc.GenExamples.Count, "alreadyHasExample", hasExample)
		for range cmdConfig.Doc.GenExamples.Count {
			...
			if err != nil {
				logger.Error("Error occurred while generating example for node, skipping", "path", t, "error", err)
				continue
			}
			logger.Trace("Generated example", "path", t.Path(), "example", example)
			if example == nil {
				logger.Debug("Example generation returned nil, skipping", "path", t)
