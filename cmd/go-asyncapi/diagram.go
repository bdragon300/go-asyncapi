...
type DiagramCmd struct {
	...
	MultipleFiles     bool   `arg:"--multiple-files" help:"Generate multiple diagram files, one per each source AsyncAPI document. Discards the --output option effect"`
	DisableFormatting bool   `arg:"--disable-formatting" help:"Disable d2 file formatting"`
	...
	NoDocumentBorders bool `arg:"--no-document-borders" help:"Do not draw document borders in the diagram"`
...
type DiagramCmdD2Options struct {
	...
	ThemeID     *int64   `arg:"--d2-theme-id" help:"D2 theme ID" placeholder:"ID"`
	DarkThemeID *int64   `arg:"--d2-dark-theme-id" help:"D2 dark theme ID" placeholder:"ID"`
	Pad         *int64   `arg:"--d2-pad" help:"D2 diagram padding" placeholder:"PIXELS"`
	Sketch      *bool    `arg:"--d2-sketch" help:"Render D2 diagram in sketchy style"`
	Center      *bool    `arg:"--d2-center" help:"Center the D2 diagram in the output canvas"`
	Scale       *float64 `arg:"--d2-scale" help:"D2 scale factor" placeholder:"FACTOR"`
...
type DiagramCmdD2ELKOptions struct {
	...
	NodeSpacing     *int64 `arg:"--d2-elk-node-spacing" help:"ELK engine: the spacing to be preserved between any pair of nodes of two adjacent layers" placeholder:"PIXELS"`
	Padding         string `arg:"--d2-elk-padding" help:"ELK engine: the expression of padding to be left to a parent element’s border when placing child elements. E.g [top=50,left=50,bottom=50,right=50]" placeholder:"PIXELS"`
	EdgeSpacing     *int64 `arg:"--d2-elk-edge-spacing" help:"ELK engine: the spacing to be preserved between nodes and edges that are routed next to the node’s layer" placeholder:"PIXELS"`
...
type DiagramCmdD2DagreOptions struct {
	NodeSep *int64 `json:"nodesep" arg:"--d2-dagre-node-sep" help:"Dagre engine: number of pixels that separate nodes horizontally" placeholder:"PIXELS"`
	EdgeSep *int64 `json:"edgesep" arg:"--d2-dagre-edge-sep" help:"Dagre engine: number of pixels that separate edges horizontally" placeholder:"PIXELS"`
}
...
func cliDiagram(cmd *DiagramCmd, globalConfig toolConfig) error {
	...
	if cmdConfig.Diagram.MultipleFiles {
		...
		artifacts := lo.Filter(rootDocument.Artifacts(), func(a common.Artifact, _ int) bool {
			return a.Selectable() && a.Visible()
		})
...
func cliDiagramMergeConfig(globalConfig toolConfig, cmd *DiagramCmd) (toolConfig, error) {
	...
	res.Diagram.NoDocumentBorders = coalesce(cmd.NoDocumentBorders, globalConfig.Diagram.NoDocumentBorders)
...
func getDiagramConfig(conf toolConfigDiagram) common.ConfigDiagram {
	return common.ConfigDiagram{
		...
		ShowDocumentBorders: !conf.NoDocumentBorders,
	}
}
...
func compileD2(contents []byte, d2opts toolConfigDiagramD2Opts) (*d2target.Diagram, *d2graph.Graph, error) {
	...
	renderOpts := getD2RenderOpts(d2opts)
...
func getD2RenderOpts(d2opts toolConfigDiagramD2Opts) d2svg.RenderOpts {
	return d2svg.RenderOpts{
		Pad:         d2opts.Pad,
		Sketch:      d2opts.Sketch,
		Center:      d2opts.Center,
		ThemeID:     d2opts.ThemeID,
