...
func (c Client) Subscriber(_ context.Context, address string, chb *runAmqp.ChannelBindings, opb *runAmqp.OperationBindings) (runAmqp.Subscriber, error) {
	...
	// If queue.is=="routingKey", the routingKey==address, the queueName==channelBindings.QueueConfiguration.Name or autogenerated.
	// If queue.is=="queue", the routingKey== "#"; the queueName==channelBindings.QueueConfiguration.Name or address.
	...
	if chb != nil {
		...
		if declare {
			_, err = ch.QueueDeclare(
				queueName,
				run.FromPtrOrZero(qc.Durable),
				run.FromPtrOrZero(qc.AutoDelete),
				run.FromPtrOrZero(qc.Exclusive),
				false,
				nil,
			)
			if err != nil {
